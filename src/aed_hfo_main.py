...
hfo_types = ["HFO", "iesHFO", "isolHFO", 'Spikes']
...
for group_label in groups:
    ...
    count_excel_path = data_path +"Biomarker_Count_Data"+os.path.sep
    os.makedirs(count_excel_path, exist_ok=True)
    count_excel_fn = count_excel_path+ group_label + "_Biomarker_Count.xlsx"
    with pd.ExcelWriter(count_excel_fn) as writer:
        ...
        for hfo_type in hfo_types:
            ...
            d = {'Patient':[], 'Pre_Cnt':[], 'Pre_Chann_Nr':[], 'Pre_Dur_S':[], 'Post_Cnt':[], 'Post_Chann_Nr':[], 'Post_Dur_S':[]}
            all_pats_df = pd.DataFrame(data=d)
            ...
            for fidx in range(len(pre_annfiles_list)):
                ...
                if hfo_type == 'HFO':
                    ...
                    pre_channels, hfo_rates_pre = get_isolhfo_rates(
                        annots_pre, eeg_duration_pre_s)
                elif hfo_type == 'Spikes':
                    pre_channels, hfo_rates_pre = get_spike_rates(
                        annots_pre, eeg_duration_pre_s)
                ...
                if hfo_type == 'HFO':
                    ...
                    post_channels, hfo_rates_post = get_spike_rates(
                        annots_post, eeg_duration_post_s)

                date_diff = eeg_data_post.info['meas_date'] - \
                            eeg_data_pre.info['meas_date']

                pre_tot_hfo_cnt = np.sum(hfo_rates_pre*eeg_duration_pre_s)
                post_tot_hfo_cnt = np.sum(hfo_rates_post*eeg_duration_post_s)


                d = {'Patient': [pre_annfiles_list[fidx]], 'Pre_Cnt': [pre_tot_hfo_cnt], 'Pre_Chann_Nr': [len(pre_channels)], 'Pre_Dur_S': [eeg_duration_pre_s],
                     'Post_Cnt': [post_tot_hfo_cnt], 'Post_Chann_Nr': [len(post_channels)], 'Post_Dur_S': [eeg_duration_post_s]}
                this_pat_df = pd.DataFrame(data=d)
                all_pats_df = pd.concat([all_pats_df, this_pat_df])
            ...
            # Plot Results
            all_pats_df.to_excel(writer, sheet_name=hfo_type)
            #plot_analysis_results(group_label, all_rates_pre, all_rates_post, hfo_type, trim_ok=False)
            pass
